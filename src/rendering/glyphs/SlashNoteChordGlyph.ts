...
export class SlashNoteChordGlyph extends ScoreNoteChordGlyphBase {
        ...
        if (this._noteGlyphLookup.has(note.id)) {
            ...
            switch (requestedPosition) {
                case NoteYPosition.TopWithStem:
                    pos -= (this.renderer as SlashBarRenderer).getStemSize(this.beamingHelper);
                    break;
    ...
    public addNoteGlyph(noteGlyph: EffectGlyph, note: Note): void {
        super.add(noteGlyph, 0);
    ...
    public updateBeamingHelper(cx: number): void {
        if (this.beamingHelper) {
            this.beamingHelper.registerBeatLineX(
                'score',
                this.beat,
                cx + this.x + this.upLineX + this.maxNote!.glyph.width,
    ...
    public paint(cx: number, cy: number, canvas: ICanvas): void {
        ...
        let scoreRenderer: SlashBarRenderer = this.renderer as SlashBarRenderer;
        //
